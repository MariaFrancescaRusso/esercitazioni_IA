...
public class SottoQActions implements ActionsFunction
{
    ...
    public Set<Action> actions(Object state)
    {
        ...
        int maxR = qState.getMaxRiga();
        int maxC = qState.getMaxColonna();
        ...
        //per velocizzare gli algoritmi di ricerca potrei dividere la matrice in sottomatrici
        if (maxR>5 && maxC>5)
        {
            int numMatrici = (maxR/5) + (maxC/5); //se 10x10 => 4 matrici
            int maxNum = (maxR*maxC)/numMatrici; //se 10x10 => 25   (cambierà in 50, 75 e 100)


            while (maxNum<qState.getLastNumber())
                maxNum += maxNum;

            if (qState.getLastNumber() == maxNum)
            {
                maxRiga = maxR;
                maxColonna = maxC;
            }
            else
            {
                if (qState.isComplete(1, 5, 1, 5))
                {
                    ...
                    if(maxNum == 75)
                    {
                        ...
                        //matrice diagonale
                        if (qState.isComplete(6, 10, 6, 10))
                        {
                            //matrice est
                            if (riga > 5 && colonna < 6) {
                                initRiga = 6;
...
//        if (riga>=1 && riga<=maxRiga && colonna>=1 && colonna<=maxColonna)
        if (riga>=1 && riga<=maxR && colonna>=1 && colonna<=maxC)
        {
            /* azione N */
//            if (riga!=1 && riga!=2 && riga!=3)
            if (riga!=initRiga && riga!=(initRiga+1) && riga!=(initRiga+2))
...
//            if (colonna!=1 && colonna!=2 && colonna!=3)
            if (colonna!=initColonna && colonna!=(initColonna+1) && colonna!=(initColonna+2))
                if (qState.getMatrice()[riga][colonna - 3] == 0)
                    actionSet.add(O);

            /* azione DNE */
//            if (riga!=1 && riga!=2 && colonna!=(maxColonna-1) && colonna!=maxColonna)
            if (riga!=initRiga && riga!=(initRiga+1) && colonna!=(maxColonna-1) && colonna!=maxColonna)
                if (qState.getMatrice()[riga - 2][colonna + 2] == 0)
                    actionSet.add(DNE);

            /* azione DNO */
//            if (riga!=1 && riga!=2 && colonna!=1 && colonna!=2)
            if (riga!=initRiga && riga!=(initRiga+1) && colonna!=initColonna && colonna!=(initColonna+1))
                if (qState.getMatrice()[riga - 2][colonna - 2] == 0)
                    actionSet.add(DNO);
...
//            if (riga!=(maxRiga-1) && riga!=maxRiga && colonna!=1 && colonna!=2)
            if (riga!=(maxRiga-1) && riga!=maxRiga && colonna!=initColonna && colonna!=(initColonna+1))
                if (qState.getMatrice()[riga + 2][colonna - 2] == 0)
                    actionSet.add(DSO);
        }
    ...
    private String cambiaLimitiMatrice(int riga, int colonna)
    {
        String tipoMatrice = "";

        //matrice est
        if (riga>5 && colonna<6)
            tipoMatrice += "MATRICEEST";

        //matrice sud
        if (riga<6 && colonna>5)
            tipoMatrice += "MATRICESUD";

        //matrice diagonale
        if (riga>5 && colonna>5)
            tipoMatrice += "MATRICEDIAGONALE";
